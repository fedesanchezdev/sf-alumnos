                    ...
                    <div className="space-y-2">
                      {(() => {
                        ...
                        const obrasAgrupadas = resumen.obrasEstudiadas.reduce((grupos, obra, index) => {
                          const compositor = obra.partitura?.compositor || obra.compositor;
                          const obraTitle = obra.partitura?.obra || obra.obra;
                          const clave = `${compositor}-${obraTitle}`;
                          if (!grupos[clave]) {
                            grupos[clave] = {
                        ...
                        return Object.values(obrasAgrupadas).map((grupo, grupoIndex) => (
                          <div key={grupoIndex} className="bg-indigo-50 p-2 rounded-md">
                            ...
                            {grupo.movimientos.some(m => m.movimientosCompases || m.comentarios) && (
                              <div className="mt-1 space-y-1">
                                {grupo.movimientos.map((movimiento, movIndex) => (
                                  <div key={movIndex} className="text-xs text-indigo-700">
                                    {movimiento.movimientosCompases && (
                                      <div>📍 {movimiento.movimientosCompases}</div>
                                    )}
                                    {movimiento.comentarios && (
                                      <div>💬 {movimiento.comentarios}</div>
                                    )}
                                  </div>
                                ))}
                              </div>

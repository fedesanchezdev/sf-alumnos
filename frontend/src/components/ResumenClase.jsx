            ...
            <div>
              ...
              <div className="space-y-3">
                {(() => {
                  ...
                  const obrasAgrupadas = formData.obrasEstudiadas.reduce((grupos, obra, index) => {
                    const clave = `${obra.compositor}-${obra.obra}`;
                    if (!grupos[clave]) {
                      grupos[clave] = {
                        compositor: obra.compositor,
                        obra: obra.obra,
                        esManual: obra.esManual,
                        partitura: obra.partitura,
                        movimientos: []
                      };
                    }
                    grupos[clave].movimientos.push({
                      ...obra,
                  ...
                  return Object.values(obrasAgrupadas).map((grupo, grupoIndex) => (
                    <div key={grupoIndex} className={`border rounded-lg p-4 bg-white ${grupo.movimientos.length > 1 ? 'border-blue-300 bg-blue-50' : 'border-gray-300'}`}>
                      {/* Encabezado del grupo (compositor y obra) */}
                      <div className="mb-3 pb-2 border-b border-gray-200">
                        <h4 className="font-semibold text-gray-900 flex items-center gap-2">
                          ...
                          {!grupo.esManual && grupo.partitura && (
                            <span className="bg-green-500 text-white text-xs px-2 py-1 rounded-full" title="Obra de la biblioteca">
                              📚 Biblioteca
                            </span>
                          )}
                        </h4>
                        <p className="text-gray-700 text-lg">{grupo.obra}</p>
                      </div>

                      {/* Movimientos del grupo */}
                      <div className="space-y-3">
                        {grupo.movimientos.map((movimiento, movIndex) => (
                          <div key={movimiento.id || movimiento.indiceOriginal} className="bg-white rounded-md p-3 border border-gray-200">
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-gray-600">
                                  {movimiento.movimientosCompases || 'Movimiento sin especificar'}
                                </span>
                              </div>
                              <button
                                type="button"
                                onClick={() => eliminarObra(movimiento.indiceOriginal)}
                            ...
                            {/* Campos editables para cada movimiento */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                  Parte de la obra
                                ...
                                <input
                                  type="text"
                                  value={movimiento.movimientosCompases || ''}
                                  onChange={(e) => actualizarObra(movimiento.indiceOriginal, 'movimientosCompases', e.target.value)}
                              ...
                              <div>
                                ...
                                <textarea
                                  ...
                                  onChange={(e) => actualizarObra(movimiento.indiceOriginal, 'comentarios', e.target.value)}
